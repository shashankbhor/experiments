...
def buildModel(input_shape,lstm_layers=3,drop_out=0.2,lstm_units=75,pred_size=1,optimizer='adam'):
    if(lstm_layers<3):
        print("Needs minimum 3 layers so defaulting with 3")
        lstm_layers=3
    regressor = Sequential()    
    for layer in np.arange(1,lstm_layers):
        if(layer==1):
